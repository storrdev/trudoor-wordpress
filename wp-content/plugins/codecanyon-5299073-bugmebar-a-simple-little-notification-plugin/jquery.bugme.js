...
(function ($) {
    $.fn.bugme = function (options) {
        var defaults = {
            ...
            minimumWidth: 0 // minimum width at which to show BMB
        };
        ...
        return this.each(function () {
            ...
            var minimumWidth = options.minimumWidth;
           ...
           if(minimumWidth > 0)
           {
            ...
            {                
                if(jQuery(window).width() < minimumWidth)
                {
                    if(_visible)
                    {
                        if ( fixedSpacer && fixed ) {
                            var bugmeHeight = jQuery('.bugme' , target).outerHeight();
                            // add to the targets margin
                            jQuery(target).animate({ marginTop: '-=' + bugmeHeight }, 50);
                        }
                        jQuery('.bugme').hide();
                        _visible = false;
                    }
